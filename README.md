...
| `video_to_video_music(video_path? \| video_url?, prompt?, segments?, ducking?, preserve_speech?, variants_num?, output_directory?)` | Generate music for a video and return a new `.mp4` with it muxed in. By default the result keeps the source's speech with the music ducked under it — pass `ducking=false` for music-only audio. Input must carry H.264/HEVC/VP9/AV1 (mp4, mov, m4v, webm); animated gif and VP8 webm are rejected. Max video duration **360s (6 min)**. `variants_num` (1–10, default 1) saves one `.mp4` per variant. | ✅ |
...
| `video_to_video_sound(video_path? \| video_url?, music_prompt?, sfx_prompt?, segments?, preserve_speech?, ducking?, variants_num?, output_directory?)` | Same as `video_to_sound`, but returns a new `.mp4` with the mixed soundtrack muxed in. Max video duration **180s (3 min)**. `variants_num` (1–10, default 1) saves one `.mp4` per variant. | ✅ |
