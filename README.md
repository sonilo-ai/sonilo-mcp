...
| `video_to_video_music(video_path? \| video_url?, prompt?, preserve_speech?, variants_num?, output_directory?)` | Generate music for a video and return a new `.mp4` with it muxed in. Max video duration **360s (6 min)**. `variants_num` (1–10, default 1) saves one `.mp4` per variant. | ✅ |
