...
def _as_number(value: object) -> float | None:
    """Read a numeric field that may arrive as a string.

    The dubbing pipeline stores its subtitle numbers in DynamoDB, which hands
    them back as strings on a finished task ("0.6305176995017312"), while the
    same fields are real numbers on the acceptance response. Returns None for
    anything that is not a number in either form, so a surprising shape
    degrades to "not reported" instead of raising on a task already paid for.
    """
...
async def _save_dubbing_artifacts(
    ...
    local files — one `.mp4` per requested language, plus one `.srt` per
    language when the call asked for re-timed subtitles.
    ...
    subtitle_urls = (
        {
            language: url
            for language, url in subs.items()
            if isinstance(url, str) and url
        }
        if isinstance(subs, dict)
        else {}
    )
    exports = body.get("subtitle_export")
...
@mcp.tool(
    description=(
        ...
        "source voice at finalize time. Default-OFF server-side: leave "
        "unset to skip it, pass True to run it, which adds a ducked track "
        "alongside the unchanged clean audio. Free, best-effort.\n"
        "    variants_num (int, optional): 1-10, default 1. Generate this "
        "many distinct music variants in one request — each is its own "
        "creative direction with its own title, not just a re-roll. Cost "
        ...
        "combined result).\n"
        "    ducking=True: also one TextContent per ducked stream (music "
        "lowered under the source voice), when the backend rendered one.\n"
...
@mcp.tool(
    description=(
        ...
        "music statically mixed under it, adding ducking=True to dip the music "
        "under the voice instead; or preserve_speech=True to keep only the "
        "isolated speech. "
        ...
        "are COMBINED — not whether a voice is kept. Default False, matching "
        "the backend: a static voice-forward mix at a fixed offset. Pass True "
        "for the dynamic duck, where the music dips only while the voice is "
        "present. Has no "
...
async def video_to_video_music(
    ...
    # `ducking` and `keep_original_sound` are both default-OFF, so each is sent
    # only to opt IN. Sending the matching value explicitly would be harmless
    # but pointless; omitting a differing value would silently give the caller
    # the wrong audio.
...
async def _stage_subtitles(
    ...
    """Turn {language: script} into the multipart parts /v1/dubbing parses.

    Returns `(files, form_fields)`, both keyed `subtitles[<language>]` — the
    bracketed field name the backend requires. A value starting with
    `https://` rides as a text field; anything else is a local path, read and
    uploaded as a file part. That split is unambiguous (a real path cannot
    start with `https://`) and matches how `video` already accepts a path.

    This server runs on the caller's own machine and already uploads their
    video, so a path is the natural way to hand over a script — unlike the
    hosted server, which can only take URLs.

    Only the two checks the server cannot make on the caller's behalf happen
    here: the suffix (a `.txt` script is a guaranteed 422 after an upload)
    and, via _resolve_input_file, that the file exists inside the allowed
    ...
    for language, value in subtitles.items():
        ...
        if not isinstance(value, str) or not value.strip():
            raise Exception(
                f"{field} must be an https:// URL or a path to a local "
                ".srt/.vtt file"
            )
        if value.lower().startswith("https://"):
            form[field] = value
            continue
        # Checked before _resolve_input_file so a wrongly-typed script is
        # named as such, rather than reported as a missing file when the
        # caller also got the path wrong.
        if Path(os.path.expanduser(value)).suffix.lower() not in _SUBTITLE_EXTS:
            raise Exception(
...
async def _submit_video_to_sound(
    *,
    ...
    if preserve_speech:
        form["preserve_speech"] = "true"
    # Always sent explicitly. The backend default is OFF, so "false" is the
    # no-op and "true" is the opt-in — sending it either way keeps the tool's
    # behavior identical to what its schema advertises. On /v1/video-to-sound
    # this flag does double duty: with no keep_original_sound field there, it
    # picks the voice source as well as the mix, so "false" is what leaves the
    # source's own speech out of the deliverable entirely.
    form["ducking"] = "true" if ducking else "false"
    # Audio endpoint only: video_to_video_sound always muxes the mix into an
    # mp4, so that tool never passes this and the field never goes out.
    if output_format:
...
@mcp.tool(
    description=(
        ...
        "    ducking (bool, optional): Bring the source video's own speech "
        "into the mix and dip the generated music under it. Defaults to "
        "False, and this endpoint has no keep_original_sound, so by default "
        "the result is the generated music and effects ALONE — the source's "
        "speech is not in it and there is no music_processed stem. Pass "
        "preserve_speech instead to bring in the isolated speech rather than "
        "the whole track.\n"
...
@mcp.tool(
    description=(
        ...
        "are COMBINED — not whether a voice is kept. Defaults to False: a "
        "static voice-forward mix at a fixed offset. Pass True for the "
        "dynamic duck, where the bed dips only while the voice is present. "
        "Has no effect when there is no "
...
@mcp.tool(
    description=(
        ...
        'es_419, de, fr, it, ru, th, ar, tr, vi, id. pt_br is Brazilian '
        ...
        'country dialect. Omit to get the default ["zh_cn", "es", "fr"].\n'
        ...
        "to SONILO_MCP_BASE_PATH) or an https URL. These are "
        "TARGET-language scripts carrying the lines to be spoken in that "
        "language — not a transcript of the source — and they replace the "
        "pipeline's own translation. The keys must match the languages "
        "being dubbed exactly; the backend rejects a missing or extra "
        "one.\n"
        "    export_srt (bool, optional): After the dub, align each "
        "language's final audio against its script and also return a "
        "re-timed .srt keeping those lines verbatim. Requires subtitles. "
        "Off by default.\n"
        ...
        "dubbing-<first 8 chars of the task id>.<language>.mp4, plus "
        "dubbing-<first 8 chars of the task id>.<language>.srt when "
        "export_srt was used — each reported with that language's export "
        "status and alignment loss. A language whose export was blocked "
        "still gets its dubbed video and is reported as a note, not a "
        "failure. On timeout the error message includes the task_id — "
        "recover with get_sfx_task."
    )
)
async def dubbing(
    video_path: str | None = None,
    ...
    if export_srt is not None:
        form["export_srt"] = "true" if export_srt else "false"
